/***************************************************************
 * Table for relayed domains
 *
 * @author Gerolf Vent <dev@gerolfvent.de>
 ***************************************************************/

CREATE TABLE domains_relayed (
    ID INT PRIMARY KEY
    ...
    transport_id INT NOT NULL
        REFERENCES transports(ID)
            ON DELETE RESTRICT
            ON UPDATE CASCADE,
    enabled BOOLEAN NOT NULL DEFAULT(true),  -- Whether relaying this domain is enabled
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    deleted_at TIMESTAMPTZ
);

CREATE INDEX idx_domains_relayed_fqdn ON domains_relayed(fqdn) WHERE deleted_at IS NULL;
CREATE INDEX idx_domains_relayed_transport_id ON domains_relayed(transport_id) WHERE deleted_at IS NULL;
CREATE INDEX idx_domains_relayed_enabled ON domains_relayed(enabled) WHERE deleted_at IS NULL;

CREATE TRIGGER trigger_updated_at
    BEFORE UPDATE ON domains_relayed
    FOR EACH ROW
    EXECUTE FUNCTION hook_update_updated_at();

CREATE TRIGGER trigger_audit
    AFTER INSERT OR UPDATE OR DELETE ON domains_relayed
    FOR EACH ROW
    EXECUTE FUNCTION hook_audit();

CREATE TRIGGER trigger_register_shared_id
    BEFORE INSERT OR UPDATE OR DELETE ON domains_relayed
    FOR EACH ROW
...
CREATE TRIGGER trigger_cascade_soft_delete_domains_canonical
    AFTER UPDATE ON domains_relayed
    FOR EACH ROW
    EXECUTE FUNCTION hook_cascade_soft_delete('domains_canonical', 'target_domain_id');

CREATE TRIGGER trigger_cascade_soft_delete_recipients_relayed
    AFTER UPDATE ON domains_relayed
    FOR EACH ROW
    EXECUTE FUNCTION hook_cascade_soft_delete('recipients_relayed', 'domain_id');
...
CREATE TRIGGER trigger_check_foreign_key_soft_delete
    BEFORE INSERT OR UPDATE ON domains_relayed
    FOR EACH ROW
