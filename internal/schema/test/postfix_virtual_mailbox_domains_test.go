...
func TestPostfixVirtualMailboxDomains(t *testing.T) {
	for _, d := range fixtures.DomainsManaged {
		...
		assertPostfixVirtualMailboxDomain(t, d.FQDN, expectRow)
	}
}

func assertPostfixVirtualMailboxDomain(t *testing.T, fqdn string, expectRow bool) {
	t.Helper()
